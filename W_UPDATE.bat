...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { if (-not (Get-PackageProvider -Name NuGet -ErrorAction SilentlyContinue)) { Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force -Scope CurrentUser; exit 0 } } catch { Write-Host $_.Exception.Message; exit 1 }" >> "%LOG_FILE%" 2>&1
if %errorlevel% neq 0 goto error_provider
